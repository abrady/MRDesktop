...
bool FrameRenderer::ValidateYUVData(const std::vector<uint8_t> &yuvData,
                                    uint32_t width, uint32_t height)
{
    ...
    {
        LOGE("Invalid YUV data size: got %zu, expected %zu-%zu", yuvData.size(), minSize, maxSize);
        return false;
    }

    if (width == 0 || height == 0 || width % 2 != 0 || height % 2 != 0)
    {
        LOGE("Invalid dimensions: %ux%u (must be non-zero and even)", width, height);
        return false;
    }

    LOGI("YUV validation passed: %ux%u, %zu bytes (expected %zu)", width, height, yuvData.size(), expectedSize);
    return true;
}
...
void FrameRenderer::ConvertYUVPixelToRGB(uint8_t y, uint8_t u, uint8_t v,
...
bool FrameRenderer::ConvertYUVToARGB(const std::vector<uint8_t> &yuvData,
                                     uint32_t width, uint32_t height,
    ...
    try
    {
        ...
        for (uint32_t row = 0; row < height; row++)
        {
            for (uint32_t col = 0; col < width; col++)
            {
                ...
                {
                    LOGE("ARGB pixel index out of bounds: %u >= %zu", pixelIndex + 3, argbData.size());
                    return false;
                }
