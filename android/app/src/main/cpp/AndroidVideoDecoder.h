...
public:
    ...
    bool DecodeFrame(const std::uint8_t *compressedData, std::size_t dataSize, std::vector<std::uint8_t> &rgbaData);
    void Cleanup();
    ...
    // Test functionality
    bool SaveFrameAsPPM(const std::vector<std::uint8_t> &rgbaData, const std::string &filename);
    bool SaveFrameAsBMP(const std::vector<std::uint8_t> &yuvData, const std::string &filename);
};