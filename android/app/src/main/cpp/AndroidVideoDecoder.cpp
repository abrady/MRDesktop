...
bool AndroidVideoDecoder::DecodeFrame(const uint8_t *compressedData, size_t dataSize, std::vector<uint8_t> &rgbaData)
{
    ...
    try
    {
        ...
        {
            LOGE("Buffer bounds check failed: offset=%zu, size=%zu, total_buffer=%zu",
                 offsetInBuffer, bytesToCopy, outputBufferSize);
            AMediaCodec_releaseOutputBuffer(m_codec, outputBufferIndex, false);
            return false;
        }
        ...
        {
            LOGE("Invalid buffer offset: offset=%zu >= buffer_size=%zu", offsetInBuffer, outputBufferSize);
            AMediaCodec_releaseOutputBuffer(m_codec, outputBufferIndex, false);
            return false;
        }
