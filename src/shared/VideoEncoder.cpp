...
bool VideoEncoder::Initialize(
  ...
  if (compression == COMPRESSION_NONE) {
    std::cerr << "VideoEncoder: Cannot initialize with COMPRESSION_NONE"
              << std::endl;
    return false;
  }
  ...
  if (!codecName) {
    std::cerr << "VideoEncoder: Unsupported compression type: " << compression
              << std::endl;
    return false;
  }
  ...
  if (!codec) {
    std::cerr << "VideoEncoder: Could not find encoder: " << codecName
              << std::endl;
    return false;
  }
  ...
  std::cout << "VideoEncoder: Initialized " << codecName << " encoder ("
...
bool VideoEncoder::EncodeFrame(
  ...
  if (ret == AVERROR(EAGAIN)) {
    ...
    std::cerr << "VideoEncoder: Error receiving packet from encoder"
              << std::endl;
    return false;
  }
  ...
  // Check if this is a keyframe (output parameter - reports what encoder
  // actually produced)
  isKeyframe = (m_Packet->flags & AV_PKT_FLAG_KEY) != 0;

  // Debug: Log frame information
  std::cout
      << "VideoEncoder: Frame " << (m_FrameCount - 1)
      << " - PTS=" << m_Packet->pts << ", Size=" << m_Packet->size << " bytes"
