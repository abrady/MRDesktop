...
bool AsioConnection::SendFrame(const FrameMessage& frameMsg, const std::vector<uint8_t>& frameData) {
...
bool AsioConnection::SendCompressedFrame(const CompressedFrameMessage& frameMsg, const std::vector<uint8_t>& frameData) {
    if (!SendMessage(frameMsg)) {
        return false;
    }
    return SendData(frameData.data(), frameData.size());
...
bool AsioConnection::SendMouseMove(int32_t deltaX, int32_t deltaY, bool absolute, int32_t x, int32_t y) {
...
bool AsioConnection::SendMouseClick(MouseClickMessage::MouseButton button, bool pressed) {
...
bool AsioConnection::SendCompressionRequest(CompressionType compression) {
    ...
    std::cout << "AsioConnection::SendCompressionRequest: sending type=" << msg.header.type 
              << " size=" << msg.header.size << " compression=" << compression << std::endl;
...
void AsioConnection::HandleReceive(const asio::error_code& error, size_t bytesTransferred) {
    if (error) {
    ...
    if (m_readingHeader) {
        ...
        switch (m_currentHeader.type) {
            case MSG_FRAME_DATA: {
                ...
                asio::async_read(*m_socket,
                    ...
                    [this](const asio::error_code& error, size_t bytesTransferred) {
                        ...
                        memcpy(reinterpret_cast<char*>(&frameMsg) + sizeof(MessageHeader), 
                               m_receiveBuffer.data(), sizeof(FrameMessage) - sizeof(MessageHeader));
            ...
            case MSG_COMPRESSED_FRAME: {
                ...
                m_receiveBuffer.resize(sizeof(CompressedFrameMessage) - sizeof(MessageHeader));
                asio::async_read(*m_socket,
                    asio::buffer(m_receiveBuffer),
                    [this](const asio::error_code& error, size_t bytesTransferred) {
                        if (error) {
                            NotifyError("Failed to read CompressedFrameMessage: " + error.message());
                            return;
                        }
                        ...
                        memcpy(reinterpret_cast<char*>(&compMsg) + sizeof(MessageHeader), 
                               m_receiveBuffer.data(), sizeof(CompressedFrameMessage) - sizeof(MessageHeader));
                        
                        // Now read the compressed data
                        m_receiveBuffer.resize(compMsg.compressedSize);
                        m_readingHeader = false;
                        m_currentCompressedFrameMsg = compMsg; // Store for later processing
                        StartReceive();
                    });
                return;
            }
                    ...
                    asio::async_read(*m_socket,
                        ...
                        [this](const asio::error_code& error, size_t bytesTransferred) {
                            if (!error) {
                                ...
                                NotifyError("Failed to read input message: " + error.message());
                            }
                        });
                } else {
                    ProcessMessage();
...
void AsioConnection::ProcessMessage() {
    std::cout << "AsioConnection::ProcessMessage: type=" << m_currentHeader.type << std::endl;
    switch (m_currentHeader.type) {
        case MSG_FRAME_DATA: {
        ...
        case MSG_MOUSE_SCROLL:
        case MSG_COMPRESSION_REQUEST: {
            std::cout << "Processing input message: type=" << m_currentHeader.type << " callback=" << (m_onInput ? "set" : "null") << std::endl;
            if (m_onInput) {
        ...
        default:
            std::cout << "Unknown message type: " << m_currentHeader.type << std::endl;
            break;
    }
}
...
void AsioServer::StartAccept() {
    ...
    m_acceptor->async_accept(*newConnection->m_socket,
        [this, newConnection](const asio::error_code& error) {
            HandleAccept(newConnection, error);
        });
}

void AsioServer::HandleAccept(std::shared_ptr<AsioConnection> newConnection, const asio::error_code& error) {
    if (!error && m_running) {
