...
FrameLogger::FrameLogger(uint32_t maxFrames, const std::string& outputDir)
    : m_maxFrames(maxFrames), m_outputDir(outputDir), m_frameCounter(0) {
  EnsureOutputDirectory();
}

FrameLogger::~FrameLogger() {
  if (!m_loggedFrames.empty()) {
    std::cout << "FrameLogger: Auto-saving " << m_loggedFrames.size()
              << " frames to disk..." << std::endl;
    SaveFramesToDisk();
  }
}

bool FrameLogger::LogFrame(
    uint32_t width,
    uint32_t height,
    uint32_t dataSize,
  ...
  std::cout << "FrameLogger: Logged frame " << frame.frameIndex << " (" << width
...
void FrameLogger::SaveFramesToDisk() {
  ...
  if (metaFile.is_open()) {
    metaFile << "Frame Debug Log - " << m_loggedFrames.size()
             << " frames captured\n";
    metaFile
        << "Generated: "
    ...
    metaFile << std::left << std::setw(8) << "Index" << std::setw(12) << "Width"
             << std::setw(12) << "Height" << std::setw(12) << "DataSize"
    ...
    for (const auto& frame : m_loggedFrames) {
      ...
      metaFile << std::left << std::setw(8) << frame.frameIndex << std::setw(12)
               << frame.width << std::setw(12) << frame.height << std::setw(12)
               << frame.dataSize << std::setw(20) << frame.filename << timeMs
               << "ms\n";
    }
...
void FrameLogger::PrintFrameStats() const {
  ...
  std::cout << "Total frames logged: " << m_loggedFrames.size() << "/"
            << m_maxFrames << std::endl;
  ...
  std::cout << "Resolution range: " << minWidth << "x" << minHeight << " to "
...
void FrameLogger::EnsureOutputDirectory() {
  try {
    if (!std::filesystem::exists(m_outputDir)) {
      ...
      std::cout << "FrameLogger: Created output directory: " << m_outputDir
                << std::endl;
    }
  } catch (const std::exception& e) {
    std::cerr << "FrameLogger: Failed to create output directory: " << e.what()
              << std::endl;
  }
}

std::string FrameLogger::GenerateFrameFilename(
    uint32_t frameIndex, uint32_t width, uint32_t height) {
  std::ostringstream oss;
  oss << "frame_" << std::setfill('0') << std::setw(3) << frameIndex << "_"
      << width << "x" << height << ".bmp";
  return oss.str();
}