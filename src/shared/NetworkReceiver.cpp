...
public:
    ...
    // Callbacks
    std::function<void(const FrameMessage&, const std::vector<uint8_t>&)> onFrameReceived;
    ...
    Impl() {
        ...
        connection.SetFrameCallback([this](const FrameMessage& frameMsg, const std::vector<uint8_t>& frameData) {
            HandleUncompressedFrame(frameMsg, frameData);
        });
        
        connection.SetCompressedFrameCallback([this](const CompressedFrameMessage& frameMsg, const std::vector<uint8_t>& frameData) {
...
private:
    void HandleUncompressedFrame(const FrameMessage& frameMsg, const std::vector<uint8_t>& frameData) {
        if (onRawFrameReceived) onRawFrameReceived(MSG_FRAME_DATA);
    ...
    void HandleCompressedFrame(const CompressedFrameMessage& frameMsg, const std::vector<uint8_t>& compressedData) {
        if (onRawFrameReceived) onRawFrameReceived(MSG_COMPRESSED_FRAME);
        ...
        if (!decoder->IsInitialized()) {
            if (!decoder->Initialize(frameMsg.width, frameMsg.height, compression)) {
                if (onError) onError("Failed to initialize video decoder");
                return;
            }
        }
        ...
        if (decoder->DecodeFrame(compressedData.data(), compressedData.size(), decodedData)) {
            ...
            if (onError) onError("Failed to decode compressed frame");
        }
    }
};
...
bool NetworkReceiver::Connect(const std::string& serverIP, int port) {
    std::cout << "NetworkReceiver::Connect starting with compression=" << pImpl->compression << std::endl;
    ...
    if (connected) {
        ...
        if (pImpl->compression != COMPRESSION_NONE) {
            std::cout << "Sending compression request: " << pImpl->compression << std::endl;
...
void NetworkReceiver::SetFrameCallback(std::function<void(const FrameMessage&, const std::vector<uint8_t>&)> callback) {
    pImpl->onFrameReceived = callback;
}

void NetworkReceiver::SetErrorCallback(std::function<void(const std::string&)> callback) {
...
void NetworkReceiver::SetRawFrameCallback(std::function<void(MessageType)> callback) {
