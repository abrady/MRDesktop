...
void PrintUsage() {
  ...
  std::cout
      << "  --compression=<none|h264|h265|av1>  Preferred compression (default: h265)"
      << std::endl;
  std::cout
      << "  --debug-frames[=N] Save first N frames for debugging (default: 5)"
      << std::endl;
  std::cout
      << "  --test             Run in test mode (validate frames and exit)"
      << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  if (debugFrames) {
    std::cout << "Debug mode: Will save first " << maxDebugFrames << " frames"
              << std::endl;
  }
  if (testMode) {
    std::cout
        << "TEST MODE: Will validate frames and exit after receiving 3 frames"
        << std::endl;
  }
  std::cout << std::endl;
  ...
  if (debugFrames) {
    ...
    std::cout << "Frame logging enabled - will save to debug_frames/ directory"
              << std::endl;
  }
  ...
  receiver.SetFrameCallback([&](const FrameMessage& frameMsg,
                                const std::vector<uint8_t>& frameData) {
    ...
    if (testMode) {
      // All frames in the callback have type MSG_FRAME_DATA (either original or
      // decoded) We need a different way to detect if compression was actually
      // used
      std::cout
          << "TEST: Received frame " << frameCount << " - " << frameMsg.width
          << "x" << frameMsg.height << " (" << frameData.size()
          << " bytes) [Type: " << frameMsg.header.type << "]" << std::endl;

      // Validate expected dimensions
      if (frameMsg.width != 640 || frameMsg.height != 480) {
        std::cerr << "TEST FAILED: Expected 640x480, got " << frameMsg.width
                  << "x" << frameMsg.height << std::endl;
        testPassed = false;
      }
      ...
      if (frameData.size() != expectedSize) {
        std::cerr << "TEST FAILED: Expected " << expectedSize << " bytes, got "
      ...
      if (frameData.size() >= expectedSize) {
        // Check top-left corner (should be mostly black with blue frame
        // counter)
        ...
        if (alpha != 255) {
          std::cerr << "TEST FAILED: Alpha channel not 255 at (0,0)"
                    << std::endl;
          testPassed = false;
        }

        std::cout << "TEST: Frame " << frameCount
                  << " pixel (0,0) = R:" << (int)red << " G:" << (int)green
                  << " B:" << (int)blue << " A:" << (int)alpha << std::endl;
      }
    ...
    if (frameLogger && frameLogger->IsLogging()) {
      ...
      if (!frameLogger->IsLogging()) {
        ...
        std::cout
            << "Frame logging complete. Files saved to debug_frames/ directory."
            << std::endl;
      }
    }
    ...
    if (frameCount % 60 == 0) {
      ...
      std::cout << "Status: " << frameCount << " frames, FPS: " << fps
                << ", Resolution: " << frameMsg.width << "x" << frameMsg.height
                << std::endl;
    }
  });
  ...
  std::cout << "Streaming ended. Total frames received: " << frameCount
            << std::endl;

  if (testMode) {
    std::cout << std::endl << "=== COMPRESSION TEST RESULTS ===" << std::endl;
    std::cout << "Requested compression: " << compression << std::endl;
    std::cout << "Received compressed frames: "
              << (receivedCompressedFrame ? "YES" : "NO") << std::endl;
    std::cout << "Received uncompressed frames: "
              << (receivedUncompressedFrame ? "YES" : "NO") << std::endl;
    std::cout << "Total frames: " << frameCount << std::endl;

    // Test validation logic
    if (compression != COMPRESSION_NONE && !receivedCompressedFrame) {
      std::cout << "TEST FAILED: Requested compression type " << compression
                << " but server sent uncompressed frames!" << std::endl;
      std::cout
          << "This indicates compression encoder is not available or failed to initialize."
          << std::endl;
      testPassed = false;
    } else if (compression == COMPRESSION_NONE && receivedCompressedFrame) {
      std::cout
          << "TEST FAILED: Requested no compression but received compressed frames!"
          << std::endl;
      testPassed = false;
    }
    ...
    if (testPassed && frameCount >= 3) {
      if (compression != COMPRESSION_NONE && receivedCompressedFrame) {
        std::cout << "TEST PASSED: Compression working - " << frameCount
                  << " frames encoded/decoded successfully!" << std::endl;
      } else if (compression == COMPRESSION_NONE && receivedUncompressedFrame) {
        std::cout << "TEST PASSED: Uncompressed mode working - " << frameCount
                  << " frames received successfully!" << std::endl;
      } else {
        std::cout
            << "TEST PASSED: Frame validation passed but compression behavior unclear"
            << std::endl;
      }
      return 0; // Success
    } else {
      std::cout << "TEST FAILED: "
                << (testPassed ? "Insufficient frames received"
                               : "Frame validation or compression test failed")
                << std::endl;
      return 1; // Failure
    }
  }
