...
bool MRClient::ConnectToServer(const FString& ServerAddress, int32 Port) {
  ...
  Socket = SocketSubsystem->CreateSocket(
      NAME_Stream, TEXT("MRClient Socket"), false);
  if (!Socket) {
  ...
  if (!FIPv4Address::Parse(ServerAddress, IPv4Address)) {
    UE_LOG(
        LogTemp,
        Error,
        TEXT("MRClient: Invalid server address: %s"),
        *ServerAddress);
    Socket->Close();
  ...
  UE_LOG(
      LogTemp,
      Log,
      TEXT("MRClient: Connecting to %s:%d"),
      *ServerAddress,
      Port);
...
bool MRClient::SendMouseMove(
    int32 DeltaX, int32 DeltaY, bool bAbsolute, int32 X, int32 Y) {
  ...
  int32 BytesSent = 0;
  return Socket->Send(
             reinterpret_cast<uint8*>(&msg),
             sizeof(MouseMoveMessage),
             BytesSent) &&
...
bool MRClient::ReceiveFrameData() {
  ...
  AsyncTask(
      ENamedThreads::GameThread, [this, FrameData = MoveTemp(FrameData)]() {
...
void MRClient::CreateRemoteTexture(int32 Width, int32 Height) {
  if (RemoteTex && RemoteTex->GetSizeX() == Width &&
      RemoteTex->GetSizeY() == Height) {
    return;
  }
...
void MRClient::PushFrame(
    const uint8* Data, int32 W, int32 H, int32 PitchBytes) {
  TSharedPtr<TArray<uint8>, ESPMode::ThreadSafe> Buffer =
      MakeShared<TArray<uint8>>();
