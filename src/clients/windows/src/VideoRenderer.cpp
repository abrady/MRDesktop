...
HRESULT VideoRenderer::CreateDeviceIndependentResources() {
  ...
  if (FAILED(hr)) {
    std::cout << "Failed to create Direct2D factory. HRESULT: 0x" << std::hex
              << hr << std::dec << std::endl;
    return hr;
  }
  ...
  if (FAILED(hr)) {
    std::cout << "Failed to create DirectWrite factory. HRESULT: 0x" << std::hex
              << hr << std::dec << std::endl;
    return hr;
  }
  ...
  hr = m_pDWriteFactory->CreateTextFormat(
      ...
      &m_pTextFormat);
  if (FAILED(hr)) {
    std::cout << "Failed to create text format. HRESULT: 0x" << std::hex << hr
              << std::dec << std::endl;
    return hr;
  }
...
HRESULT VideoRenderer::CreateDeviceResources() {
  ...
  std::cout << "Creating render target for window size: " << std::dec
            << size.width << "x" << size.height << std::endl;

  // Check for invalid window size
  if (size.width == 0 || size.height == 0) {
    std::cout << "Window has zero size, cannot create render target yet"
              << std::endl;
    return E_FAIL;
  }
  ...
  if (FAILED(hr)) {
    std::cout << "Failed to create HWND render target. HRESULT: 0x" << std::hex
              << hr << std::dec << std::endl;
    return hr;
  }
  ...
  if (FAILED(hr)) {
    std::cout << "Failed to create text brush. HRESULT: 0x" << std::hex << hr
              << std::dec << std::endl;
    return hr;
  }
...
HRESULT VideoRenderer::RenderFrame(
    const FrameMessage& frameMsg, const std::vector<BYTE>& frameData) {
  ...
  if (!m_pFrameBitmap || m_pFrameBitmap->GetSize().width != frameMsg.width ||
  ...
  // Calculate scaling to fit the frame in the window while maintaining aspect
  // ratio
  ...
  m_pRenderTarget->DrawBitmap(
      m_pFrameBitmap, destRect, 1.0f, D2D1_BITMAP_INTERPOLATION_MODE_LINEAR);
...
void VideoRenderer::CalculateFPS() {
  ...
  if (m_frameCount % 10 == 0 && m_frameCount > 0) {
    double elapsedSeconds =
        static_cast<double>(currentTime.QuadPart - m_lastFrameTime.QuadPart) /
