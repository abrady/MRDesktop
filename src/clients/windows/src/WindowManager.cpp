...
LRESULT CALLBACK WindowManager::StaticWndProc(
  ...
  if (message == WM_NCCREATE) {
    ...
    pThis =
        reinterpret_cast<WindowManager*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
  }
...
void WindowManager::OnCreate() {
  ...
  if (FAILED(hr)) {
    std::cout << "GDI renderer initialization failed with HRESULT: 0x"
              << std::hex << hr << std::dec << std::endl;
    MessageBoxW(
        m_hwnd,
        L"Failed to initialize GDI video renderer.\nThis is a critical error.",
        L"Video Renderer Error",
        MB_OK | MB_ICONERROR);
    PostQuitMessage(1);
    return;
  }
  ...
  std::cout
      << "GDI renderer initialized. Will attempt Direct2D upgrade when window is ready."
      << std::endl;
  ...
  m_networkConnection->SetFrameCallback(
      [this](
  ...
  m_inputHandler->SetMouseClickCallback([this](int button, bool pressed) {
    if (m_networkConnection && m_networkConnection->IsConnected()) {
      MouseClickMessage::MouseButton btn =
          static_cast<MouseClickMessage::MouseButton>(button);
      m_networkConnection->SendMouseClick(btn, pressed);
    }
  });

  m_inputHandler->SetMouseScrollCallback(
      [this](int32_t deltaX, int32_t deltaY) {
        if (m_networkConnection && m_networkConnection->IsConnected()) {
...
void WindowManager::ShowConnectionDialog() {
  ...
  m_statusMessage = "Press SPACE to connect to " + m_serverIP + ":" +
...
void WindowManager::OnFrameReceived(
    const FrameMessage& frameMsg, const std::vector<uint8_t>& frameData) {
  // Try to upgrade to Direct2D renderer if we're still using GDI and haven't
  // tried yet
  static bool triedDirect2DUpgrade = false;
  if (m_usingSimpleRenderer && !triedDirect2DUpgrade) {
    triedDirect2DUpgrade = true;
    std::cout
        << "Attempting to upgrade to Direct2D renderer now that window is ready..."
        << std::endl;
    ...
    if (SUCCEEDED(hr)) {
      ...
      std::cout << "Direct2D upgrade failed with HRESULT: 0x" << std::hex << hr
                << std::dec << ". Continuing with GDI renderer." << std::endl;
    }
  }
  ...
  if (frameCount % 60 == 0) {
    ...
    double actualFPS =
        60000.0 / elapsed.count(); // 60 frames in elapsed milliseconds
    std::cout << "Frame receive rate: " << std::fixed << std::setprecision(1)
...
void WindowManager::UpdateWindowTitle() {
  ...
  if (m_connectionState == ConnectionState::Connected) {
    ...
    swprintf_s(
        stats,
        L" - Connected%s - %.1f FPS - %ux%u - %u frames",
        m_usingSimpleRenderer ? L" (GDI)" : L" (D2D)",
        fps,
        width,
        height,
        frames);
...
void WindowManager::ToggleFullscreen() {
  if (!m_isFullscreen) {
    ...
    SetWindowLong(
        m_hwnd, GWL_STYLE, m_windowedStyle & ~(WS_CAPTION | WS_THICKFRAME));
    SetWindowPos(
        m_hwnd,
        HWND_TOP,
        mi.rcMonitor.left,
