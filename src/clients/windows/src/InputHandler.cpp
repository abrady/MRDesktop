...
InputHandler::InputHandler()
    : m_hwnd(nullptr), m_isCapturingMouse(false), m_lastMousePos({0, 0}) {}
...
LRESULT InputHandler::HandleMouseMove(WPARAM /*wParam*/, LPARAM lParam) {
  if (!m_isCapturingMouse || !m_onMouseMove)
    return 0;
...
LRESULT InputHandler::HandleMouseButton(
    UINT message, WPARAM /*wParam*/, LPARAM /*lParam*/) {
  if (!m_isCapturingMouse || !m_onMouseClick)
    return 0;
...
LRESULT InputHandler::HandleMouseWheel(WPARAM wParam, LPARAM /*lParam*/) {
  if (!m_isCapturingMouse || !m_onMouseScroll)
    return 0;
...
LRESULT InputHandler::HandleKeyboard(
    UINT message, WPARAM wParam, LPARAM /*lParam*/) {
  if (!m_onKeyPress)
    return 0;
