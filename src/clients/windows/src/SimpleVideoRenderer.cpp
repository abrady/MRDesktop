...
HRESULT SimpleVideoRenderer::RenderFrame(
  ...
  if (!m_bitmap || m_bitmapWidth != frameMsg.width ||
    ...
    void* pBits = nullptr;
    m_bitmap =
        CreateDIBSection(m_memDC, &bmi, DIB_RGB_COLORS, &pBits, nullptr, 0);
    if (!m_bitmap) {
      return E_FAIL;
    }
  ...
  sprintf_s(
      statsText,
      "FPS: %.1f | Frames: %u | Resolution: %ux%u | Data: %.1f MB/s",
      m_fps,
      m_frameCount,
      m_currentWidth,
      m_currentHeight,
